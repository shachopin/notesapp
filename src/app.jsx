...
class App extends React.Component {
  ...
  newNote = async (title) => {
    ...
    //IMPORTANT,  the reason for above is bcause await NOT BLOCKING MAIN TRREAD EVEN IT LOOKS LIKE BLOCKING
    ...
    */
    ...
    //you shouldn't havee await before this.setState anyeay, not returnign Promis
  ...
  deleteNote = (note) => {
    ...
    if (noteToBeDeletedIndex === this.state.selectedNoteIndex) {
      //如果删除的是本来选中的，那就清空, 删除以后就什么也不选中，造成Quill hidden
      this.setState({ selectedNoteIndex: null, selectedNote: null });
    } else if (this.state.selectedNote) {
      //如果删除的不是本来选中的，那就本来选中的仍然选中, 但前提是要有选中
      const prevSelectedNoteId = this.state.selectedNote.id;
      const otherNotes = this.state.notes.filter((_note) => _note !== note);
      this.setState({
        selectedNoteIndex: otherNotes.indexOf(
          otherNotes.filter((_note) => _note.id === prevSelectedNoteId)[0]
        ),
      });
    }
    //就算本来没有任何的note被选中，依然work
    db.collection("notes").doc(note.id).delete(); //利用firebase自己会update component local state
  };
}
